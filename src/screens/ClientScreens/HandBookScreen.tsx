...
const HandBookScreen = () => {
  ...
  const column: any = 'TranslateHandbook'; // PrivatePolice BehaviorPolice
  ...
  return (
    <View style={{...styles.container}}>
      <Header
        headerTitle={t('common:translation_handbook')}
        showleftIcon
        showRightIcon
      />
      {isLoading && <CustomLoader color={colors.main} />}

      <View style={{padding: spacing.ten, marginBottom: 100}}>
        <ScrollView>
          {data?.map((item, index) => {
