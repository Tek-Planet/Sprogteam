...
import {Button, CustomCheckBox, Loader, ProfileHeader} from '../../components';
...
const ViewGigScreen = ({route}) => {
  ...
  const [translatorInfo, setTranslatorInfo] = useState(
    route.params.translatorInfo ? route.params.translatorInfo : null,
  );
  ...
  useEffect(() => {
    // tweek price
    setSelectedPrice(item?.package[selected]?.PPPrice);
  ...
  // get translator info it it is empty
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const response = await getUser(userId);
      setTranslatorInfo(response.profile);
      setLoading(false);
    };
