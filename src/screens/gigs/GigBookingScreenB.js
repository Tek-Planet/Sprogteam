...
const GigBookingScreenB = ({navigation, route}) => {
  ...
  const saveBooking = async () => {
    ...
    if (date === null) setError('Provide deadline date');
    else if (startDate === null) setError('Provide deadline time');
    else if (price <= 4) {
  ...
  const bookingObject = async () => {
    try {
      const mergedDate = await mergeDateTime(date, startDate, startDate);
            ...
            <View style={{marginTop: 20}}>
              {checkPhone && (
                <View style={[styles.rowApart, {paddingHorizontal: 5}]}>
                  ...
                  <Text style={styles.text}>
                    {selectedPrice + ' ' + baseCurrency.usd}
                  </Text>
                </View>
              )}
            </View>
...
export default GigBookingScreenB;
